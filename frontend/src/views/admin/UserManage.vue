    ...
    <div class="user-layout">
      ...
      <div class="content-panel">
        ...
        <el-table :data="tableData" v-loading="loading" style="width:100%" border size="small">
          ...
          <el-table-column prop="roleName" label="绑定角色" width="90" />
          <el-table-column label="性别" width="50"><template #default="{row}">{{ row.sex || '-' }}</template></el-table-column>
...
async function handleSubmit(){
  ...
  try{
    await (isEdit.value?updateUserInfo:addUser)({
      ...form,
      account: form.email, // prototype: 邮箱作为账号
      upwd: isEdit.value ? undefined : '888itcast.CN764%...'
    })
    ElMessage.success(isEdit.value?'修改成功':'新增成功，默认密码为888itcast.CN764%...')
...
.tree-panel { width: 220px; flex-shrink: 0; border-right: 1px solid #e7e9ed; padding-right: 16px; }
