...
public class FamilyController {
    ...
    public Map<String, Object> home(HttpSession session) {
        ...
        try {
            List<Map<String, Object>> elders = jdbcTemplate.queryForList(
                "SELECT e.id, e.name, e.elder_no AS elderNo, e.gender, e.status, e.ability_level AS abilityLevel, " +
                "fe.relation, e.health_status AS healthStatus " +
                "FROM t_family_elder fe JOIN t_elder e ON fe.elder_id = e.id " +
                "WHERE fe.family_id = ? AND fe.status = 1", familyId);
            result.put("elders", elders);
        } catch (Exception e) {
            result.put("elders", new ArrayList<>());
        ...
        try {
            Integer count = jdbcTemplate.queryForObject(
                "SELECT COUNT(*) FROM t_visit WHERE elder_id IN (SELECT elder_id FROM t_family_elder WHERE family_id = ?)",
                Integer.class, familyId);
            result.put("appointmentCount", count != null ? count : 0);
        } catch (Exception e) {
            result.put("appointmentCount", 0);
        }

        // 养老院信息
        Map<String, Object> info = new HashMap<>();
        info.put("name", "中州养老");
        info.put("address", "河南省郑州市中州大道100号");
        info.put("phone", "0371-12345678");
        info.put("introduction", "中州养老院是一家集医疗、康复、护理、生活照料为一体的综合性养老服务机构，致力于为老年人提供温馨、舒适的居住环境和专业的护理服务。");
        result.put("nursingHome", info);
